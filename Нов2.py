...
class InequalitySystemApp:
    def __init__(self):
        # Создаем окно с тремя осями
        self.fig, (self.ax1, self.ax2, self.ax3) = plt.subplots(3, 1, figsize=(14, 8.5))
        # Оставляем место внизу для ползунков и знаков, справа для текста
        plt.subplots_adjust(left=0.08, bottom=0.3, right=0.72, hspace=0.6)
        
        # Начальные параметры
        self.val_a = -2.0
        self.val_b = 3.0
        self.sign1 = "x > a"  # Допустимые: "x > a", "x < a", "x >= a", "x <= a"
        self.sign2 = "x < b"  # Допустимые: "x > b", "x < b", "x >= b", "x <= b"
        
        # Настраиваем внешний вид осей
        self.axes = [self.ax1, self.ax2, self.ax3]
        titles = ["Неравенство №1", "Неравенство №2", "ИТОГОВОЕ РЕШЕНИЕ СИСТЕМЫ (Пересечение)"]
        ...
        for i, ax in enumerate(self.axes):
            ...
            ax.text(9.8, -0.2, 'X', fontsize=11, fontweight='bold')
            ax.text(-10, 0.8, titles[i], fontsize=11, fontweight='bold', color='#1a365d')

        # Элементы для Неравенства 1 (Ось 1)
        self.dot_a, = self.ax1.plot([], [], 'ro', markersize=10, zorder=5)
        self.lbl_a = self.ax1.text(0, -0.4, '', ha='center', fontweight='bold', color='crimson')
        self.fill1 = None
        
        # Элементы для Неравенства 2 (Ось 2)
        self.dot_b, = self.ax2.plot([], [], 'bo', markersize=10, zorder=5)
        self.lbl_b = self.ax2.text(0, -0.4, '', ha='center', fontweight='bold', color='royalblue')
        self.fill2 = None
        
        # Элементы для Итоговой оси (Ось 3)
        self.dot_res_a, = self.ax3.plot([], [], 'ro', markersize=9, zorder=5)
        self.dot_res_b, = self.ax3.plot([], [], 'bo', markersize=9, zorder=5)
        self.fill3 = None

        # Информационная панель справа (1.02 прижимает к краю экрана)
        self.info_text = self.ax3.text(1.03, 2.2, '', transform=self.ax3.transAxes, 
                                           verticalalignment='top', fontsize=11,
                                           bbox=dict(facecolor='#f9f9f9', alpha=0.95, edgecolor='gray'))
        
        # --- Интерактивные элементы (Ползунки и Кнопки знаков) ---
        ax_color = '#f5f5f5'
        
        # Ползунки для чисел a и b
        self.slider_a = Slider(plt.axes([0.15, 0.16, 0.35, 0.025], facecolor=ax_color), 'Число a', -8.0, 8.0, valinit=self.val_a, valstep=0.5)
        self.slider_b = Slider(plt.axes([0.15, 0.10, 0.35, 0.025], facecolor=ax_color), 'Число b', -8.0, 8.0, valinit=self.val_b, valstep=0.5)
        
        self.slider_a.on_changed(self.update)
        self.slider_b.on_changed(self.update)
        
        # Переключатели знаков неравенств (RadioButtons)
        # Размещаем их внизу справа от ползунков
        ax_radio1 = plt.axes([0.56, 0.04, 0.15, 0.2], facecolor='#f0f0f0')
        self.radio1 = RadioButtons(ax_radio1, ('x > a', 'x < a', 'x ≥ a', 'x ≤ a'), active=0)
        self.radio1.on_clicked(self.change_sign1)
        
        ax_radio2 = plt.axes([0.74, 0.04, 0.15, 0.2], facecolor='#f0f0f0')
        self.radio2 = RadioButtons(ax_radio2, ('x > b', 'x < b', 'x ≥ b', 'x ≤ b'), active=1)
        self.radio2.on_clicked(self.change_sign2)
        
        self.update(None)
        plt.ioff()
        plt.show()

    def change_sign1(self, label):
        self.sign1 = label
        self.update(None)

    def change_sign2(self, label):
        self.sign2 = label
        self.update(None)

    # Вспомогательный метод для удаления старой штриховки
    def clear_fill(self, fill_obj):
        if fill_obj is not None:
            for poly in fill_obj:
                poly.remove()

    # Рисует красивую школьную штриховку (наклонные линии)
    def draw_shading(self, ax, x_start, x_end, color, hatch_type='//'):
        # Ограничиваем бесконечность рамками графика от -10 до 10
        left = max(x_start, -10.0)
        right = min(x_end, 10.0)
        if left >= right:
            return None
        # Заливаем область прозрачным цветом с наклонной штриховкой
        fill = ax.fill_between([left, right], [0, 0], [0.5, 0.5], facecolor='none', edgecolor=color, hatch=hatch_type, alpha=0.4, zorder=2)
        return fill
    ...
    def update(self, val):
        ...
        # Определяем строгость знаков (True - строгое, точка выколотая)
        is_strict1 = '≥' not in self.sign1 and '≤' not in self.sign1
        is_strict2 = '≥' not in self.sign2 and '≤' not in self.sign2
        
        # Направление луча (True - вправо, False - влево)
        dir1 = '>' in self.sign1 or '≥' in self.sign1
        dir2 = '>' in self.sign2 or '≥' in self.sign2
        
        # --- 1. Настройка точек (закрашенная или выколотая с дырочкой) ---
        if is_strict1:
            self.dot_a.set_marker('o'); self.dot_a.set_markerfacecolor('white'); self.dot_a.set_markeredgewidth(2.5)
        else:
            self.dot_a.set_marker('o'); self.dot_a.set_markerfacecolor('crimson'); self.dot_a.set_markeredgewidth(1)
        self.dot_a.set_data([a],)
        self.lbl_a.set_position((a, -0.35)); self.lbl_a.set_text(f"a = {a:.1f}")
        
        if is_strict2:
            self.dot_b.set_marker('o'); self.dot_b.set_markerfacecolor('white'); self.dot_b.set_markeredgewidth(2.5)
        else:
            self.dot_b.set_marker('o'); self.dot_b.set_markerfacecolor('royalblue'); self.dot_b.set_markeredgewidth(1)
        self.dot_b.set_data([b],)
        self.lbl_b.set_position((b, -0.35)); self.lbl_b.set_text(f"b = {b:.1f}")

        # --- 2. Отрисовка штриховок для каждого неравенства ---
        self.clear_fill(self.fill1)
        self.clear_fill(self.fill2)
        self.clear_fill(self.fill3)
        
        # Промежутки лучей [от, до]
        span1 = [a, 10.0] if dir1 else [-10.0, a]
        span2 = [b, 10.0] if dir2 else [-10.0, b]
        
        self.fill1 = self.draw_shading(self.ax1, span1[0], span1[1], 'crimson', hatch_type='//')
        self.fill2 = self.draw_shading(self.ax2, span2[0], span2[1], 'royalblue', hatch_type='\\\\')
        
        # --- 3. Вычисление пересечения (Итоговая ось) ---
        # Находим пересечение двух интервалов [span1_start, span1_end] и [span2_start, span2_end]
        res_start = max(span1[0], span2[0])
        res_end = min(span1[1], span2[1])
        
        show_res_a = False
        show_res_b = False
        
        if res_start < res_end:
            # Пересечение существует! Рисуем перекрестную штриховку
            self.fill3 = self.draw_shading(self.ax3, res_start, res_end, 'darkgeometry', hatch_type='X')
            
            # Определяем, какие граничные точки нужно показать на итоговой оси
            if res_start == a: show_res_a = True
            if res_start == b: show_res_b = True
            if res_end == a: show_res_a = True
            if res_end == b: show_res_b = True
            
            # Формируем текстовую запись ответа
            l_bracket = "(" if (res_start == a and is_strict1) or (res_start == b and is_strict2) or res_start == -10.0 else "["
            r_bracket = ")" if (res_end == a and is_strict1) or (res_end == b and is_strict2) or res_end == 10.0 else "]"
            
            l_str = "−∞" if res_start == -10.0 else f"{res_start:.1f}"
            r_str = "+∞" if res_end == 10.0 else f"{res_end:.1f}"
            answer_text = f"x ∈ {l_bracket}{l_str}; {r_str}{r_bracket}"
        else:
            answer_text = "Нет решений (Пустое множество Ø)"
            
        # Настройка видимости точек на итоговой оси
        if show_res_a:
            if is_strict1:
                self.dot_res_a.set_marker('o'); self.dot_res_a.set_markerfacecolor('white'); self.dot_res_a.set_markeredgewidth(2)
            else:
                self.dot_res_a.set_marker('o'); self.dot_res_a.set_markerfacecolor('crimson'); self.dot_res_a.set_markeredgewidth(1)
            self.dot_res_a.set_data([a], [0])
            self.dot_res_a.set_visible(True)
        else: self.dot_res_a.set_visible(False)
            
        if show_res_b:
            if is_strict2:
                self.dot_res_b.set_marker('o'); self.dot_res_b.set_markerfacecolor('white'); self.dot_res_b.set_markeredgewidth(2)
            else:
                self.dot_res_b.set_marker('o'); self.dot_res_b.set_markerfacecolor('royalblue'); self.dot_res_b.set_markeredgewidth(1)
            self.dot_res_b.set_data([b], [0])
            self.dot_res_b.set_visible(True)
        else: self.dot_res_b.set_visible(False)

        # --- 4. Обновление правой текстовой панели ---
        # Меняем знаки в красивую школьную форму записи для вывода на плашку
        display_sign1 = self.sign1.replace(">=", "≥").replace("<=", "≤")
        display_sign2 = self.sign2.replace(">=", "≥").replace("<=", "≤")
        
        self.info_text.set_text(
            f"СИСТЕМА НЕРАВЕНСТВ:\n"
            f"----------------------------------------\n"
            f"ФИГУРНАЯ СКОБКА ПРЕДПИСЫВАЕТ:\n"
            f"Найти пересечение решений!\n\n"
            f"Неравенство 1:\n"
            f"{display_sign1.replace('a', f'{a:.1f}')}\n"
            f"Точка а: {'ВЫКОЛОТАЯ (строго)' if is_strict1 else 'ЗАКРАШЕННАЯ (нестрого)'}\n\n"
            f"Неравенство 2:\n"
            f"{display_sign2.replace('b', f'{b:.1f}')}\n"
            f"Точка b: {'ВЫКОЛОТАЯ (строго)' if is_strict2 else 'ЗАКРАШЕННАЯ (нестрого)'}\n\n"
            f"----------------------------------------\n"
            f"ПРАВИЛО СБОРКИ ОТВЕТА:\n"
            f"Ищем зону, где совпали обе штриховки.\n"
            f"На нижней оси она закрашена крестиком.\n\n"
            f"ОТВЕТ СИСТЕМЫ:\n"
            f"{answer_text}"
            )
            self.fig.canvas.draw_idle()
...
class CircleLaboratoryApp:
    def __init__(self):
        # Создаем большое квадратное окно для геометрии
        self.fig, self.ax = plt.subplots(figsize=(14, 8))
        # Оставляем место внизу для трех ползунков, справа для текста
        plt.subplots_adjust(left=0.06, bottom=0.22, right=0.7, top=0.92)
        
        self.ax.set_xlim(-11, 11)
        self.ax.set_ylim(-11, 11)
        self.ax.set_aspect('equal')
        self.ax.grid(True, linestyle=':', alpha=0.5)
        self.ax.set_title("Интерактивная Лаборатория: Свойства Окружности и Круга", fontsize=14, color='#1a365d', fontweight='bold', pad=15)
        
        # --- Графические элементы на чертеже ---
        # Центр окружности (Точка O)
        self.ax.plot([0], [0], 'ko', markersize=6, zorder=10)
        self.ax.text(0.3, -0.4, 'O (Центр)', fontsize=10, fontweight='bold')
        
        # Заготовка для цветного сектора (Wedge)
        self.sector_patch = Wedge((0, 0), 0, 0, 0, facecolor='#ffe6cc', edgecolor='darkorange', lw=1.5, alpha=0.7, zorder=2)
        self.ax.add_patch(self.sector_patch)
        
        # Сама окружность (Линия)
        self.circle_line, = self.ax.plot([], [], color='dimgray', lw=2.5, zorder=3)
        
        # Линия радиуса (Отрезок из центра к началу сектора)
        self.radius_line, = self.ax.plot([], [], color='blue', lw=3, marker='o', mfc='white', zorder=5, label='Радиус (R)')
        self.lbl_radius = self.ax.text(0, 0, '', color='blue', fontsize=10, fontweight='bold', va='bottom')
        
        # Линия Хорды (зеленая линия между двумя точками)
        self.chord_line, = self.ax.plot([], [], color='green', lw=2.5, marker='o', mfc='white', zorder=4, label='Хорда')
        self.lbl_chord = self.ax.text(0, 0, '', color='green', fontsize=10, fontweight='bold', ha='center')
        
        # Линии вписанного угла (фиолетовые)
        self.in_line1, = self.ax.plot([], [], color='purple', lw=1.2, ls='--')
        self.in_line2, = self.ax.plot([], [], color='purple', lw=1.2, ls='--', label='Вписанный угол')
        
        # Текстовая панель разбора справа (1.02 прижимает к краю экрана)
        self.info_text = self.ax.text(1.03, 0.5, '', transform=self.ax.transAxes, 
                                           verticalalignment='center', fontsize=10.5,
                                           bbox=dict(facecolor='#f9f9f9', alpha=0.95, edgecolor='gray'))
        
        # --- Интерактивные Ползунки (Sliders) ---
        ax_color = '#f5f5f5'
        self.slider_r     = Slider(plt.axes([0.15, 0.13, 0.45, 0.025], facecolor=ax_color), 'Радиус R', 2.0, 9.0, valinit=5.0, valstep=0.5)
        self.slider_angle = Slider(plt.axes([0.15, 0.08, 0.45, 0.025], facecolor=ax_color), 'Угол α (градусы)', 10.0, 180.0, valinit=60.0, valstep=5)
        self.slider_chord = Slider(plt.axes([0.15, 0.03, 0.45, 0.025], facecolor=ax_color), 'Раздвинуть Хорду', 10.0, 180.0, valinit=90.0, valstep=5)
        
        self.slider_r.on_changed(self.update)
        self.slider_angle.on_changed(self.update)
        self.slider_chord.on_changed(self.update)
    ...
    def update(self, val):
        R = self.slider_r.val
        alpha = self.slider_angle.val
        chord_angle = self.slider_chord.val
        
        # --- МАТЕМАТИЧЕСКИЕ РАСЧЕТЫ БАЗЫ ---
        diameter = 2 * R
        length_circle = 2 * np.pi * R
        area_circle = np.pi * (R ** 2)
        
        # --- МАТЕМАТИЧЕСКИЕ РАСЧЕТЫ СЕКТОРА И ДУГИ ---
        # Длина дуги: L = 2 * pi * R * (alpha / 360)
        length_arc = length_circle * (alpha / 360.0)
        # Площадь сектора: S_sec = pi * R^2 * (alpha / 360)
        area_sector = area_circle * (alpha / 360.0)
        
        # Вписанный угол всегда равен половине центрального (alpha / 2)
        in_angle = alpha / 2.0
        
        # --- МАТЕМАТИЧЕСКИЕ РАСЧЕТЫ ХОРДЫ ---
        # Длина хорды по теореме синусов/косинусов: b = 2 * R * sin(chord_angle / 2)
        chord_len = 2 * R * np.sin(np.radians(chord_angle / 2.0))
        
        # --- ОБНОВЛЕНИЕ ГРАФИКИ ---
        # 1. Рисуем саму окружность
        theta = np.linspace(0, 2 * np.pi, 300)
        self.circle_line.set_data(R * np.cos(theta), R * np.sin(theta))
        
        # 2. Обновляем сектор (оранжевый кусочек пирога)
        self.sector_patch.set_radius(R)
        self.sector_patch.set_theta2(alpha)
        
        # 3. Обновляем линию радиуса (направлен на 0 градусов для удобства)
        self.radius_line.set_data([0, R],)
        self.lbl_radius.set_position((R / 2, 0.2))
        self.lbl_radius.set_text(f"R = {R:.1f}")
        
        # 4. Обновляем линию хорды.
        # Чтобы она не мешала сектору, пустим её в нижней полусфере (симметрично от -chord_angle/2 до +chord_angle/2, развернув вниз)
        ang_rad1 = np.radians(180 - chord_angle / 2.0)
        ang_rad2 = np.radians(180 + chord_angle / 2.0)
        x_ch1, y_ch1 = R * np.cos(ang_rad1), R * np.sin(ang_rad1)
        x_ch2, y_ch2 = R * np.cos(ang_rad2), R * np.sin(ang_rad2)
        
        self.chord_line.set_data([x_ch1, x_ch2], [y_ch1, y_ch2])
        # Ставим подпись хорды по центру её отрезка
        self.lbl_chord.set_position(((x_ch1 + x_ch2) / 2, ((y_ch1 + y_ch2) / 2) - 0.6))
        
        # Если угол хорды 180 градусов — это диаметр!
        if abs(chord_angle - 180.0) < 0.1:
            chord_status = f"Хорда стала ДИАМЕТРОМ! ({chord_len:.1f})"
            self.lbl_chord.set_text("ДИАМЕТР")
            self.lbl_chord.set_color('crimson')
            self.chord_line.set_color('crimson')
        else:
            chord_status = f"{chord_len:.1f}"
            self.lbl_chord.set_text("Хорда")
            self.lbl_chord.set_color('green')
            self.chord_line.set_color('green')
            
        # 5. Обновляем вписанный угол
        # Посадим вершину вписанного угла в противоположную точку окружности (угол 180 градусов от начала сектора)
        # Он опирается на те же точки сектора: (R, 0) и (R*cos(alpha), R*sin(alpha))
        x_vert, y_vert = R * np.cos(np.radians(180)), R * np.sin(np.radians(180))
        x_s1, y_s1 = R, 0.0
        x_s2, y_s2 = R * np.cos(np.radians(alpha)), R * np.sin(np.radians(alpha))
        
        self.in_line1.set_data([x_s1, x_vert, x_s2], [y_s1, y_vert, y_s2])
        
        # --- ОБНОВЛЕНИЕ ТЕКСТА НА ПРАВОЙ ПАНЕЛИ ---
        self.info_text.set_text(
            f"ГЕОМЕТРИЯ ОКРУЖНОСТИ:\n"
            f"----------------------------------------\n"
            f"Базовые элементы:\n"
            f"Радиус (R) = {R:.1f}\n"
            f"Диаметр (D = 2R) = {diameter:.1f}\n"
            f"Длина окружности (C = 2πR) = {length_circle:.2f}\n"
            f"Площадь круга (S = πR²) = {area_circle:.2f}\n\n"
            
            f"ЛИНИИ И ОТРЕЗКИ:\n"
            f"Радиус: соединяет центр и точку на окр.\n"
            f"Хорда: соединяет ДВЕ любые точки окр.\n"
            f"Текущая длина Хорды = {chord_status}\n\n"
            
            f"СЕКТОР И ДУГА (Угол α = {alpha:.0f}°):\n"
            f"Дуга: кусочек «ниточки» окружности.\n"
            f"Длина дуги (L) = {length_arc:.2f}\n"
            f"Сектор: кусочек «пирога» внутри.\n"
            f"Площадь сектора = {area_sector:.2f}\n\n"
            
            f"ВЕЛИКАЯ ТЕОРЕМА ОБ УГЛАХ:\n"
            f"Центральный угол (оранжевый) = {alpha:.0f}°\n"
            f"Вписанный угол (пунктир) = {in_angle:.1f}°\n"
            f"Правило: Вписанный угол ВСЕГДА\n"
            f"ровно в 2 раза меньше центрального,\n"
            f"если они опираются на одну дугу!"
        )
        self.fig.canvas.draw_idle()

if __name__ == '__main__':
    app = CircleLaboratoryApp()   
    plt.ioff()       
    plt.show()
...
class LogarithmSimulationApp:
    def __init__(self):
        # Создаем окно: слева график логарифмической функции, справа - разбор
        self.fig, self.ax = plt.subplots(figsize=(14, 7.5))
        # Оставляем место внизу для двух ползунков, справа для текста
        plt.subplots_adjust(left=0.08, bottom=0.2, right=0.7, top=0.9)
        
        # Начальные параметры логарифма: log_a (b) = x
        self.a_init = 2.0  # Основание
        self.b_init = 4.0  # Число под логарифмом
        
        # Настройка координатной сетки
        self.ax.set_xlim(-1, 11)
        self.ax.set_ylim(-4, 6)
        self.ax.grid(True, linestyle=':', alpha=0.6)
        self.ax.axhline(0, color='black', lw=1.2)
        self.ax.axvline(0, color='black', lw=1.2)
        self.ax.set_title("Что такое Логарифм? Интерактивный тренажёр", fontsize=14, color='#1a365d', fontweight='bold', pad=15)
        self.ax.set_xlabel("Число под логарифмом (X или b)", fontsize=10)
        self.ax.set_ylabel("Результат логарифма (Y или степень)", fontsize=10)
        
        # Графические элементы на чертеже
        self.log_curve, = self.ax.plot([], [], color='royalblue', lw=2.5, label='График y = log_a(x)')
        self.current_point, = self.ax.plot([], [], 'ro', markersize=8, zorder=5) # Точка текущего значения b
        
        # Пунктирные проекции от точки к осям
        self.proj_h = self.ax.axhline(-100, color='crimson', ls=':', lw=1.2)
        self.proj_v = self.ax.axvline(-100, color='crimson', ls=':', lw=1.2)
        ...
        # --- Интерактивные Ползунки (Sliders) ---
        ax_color = '#f5f5f5'
        # Ползунок основания 'a' (от 0.1 до 5.0, исключая 1.0 в ОДЗ)
        self.slider_a = Slider(plt.axes([0.15, 0.1, 0.45, 0.025], facecolor=ax_color), 'Основание a', 0.2, 5.0, valinit=self.a_init, valstep=0.1)
        # Ползунок числа под логарифмом 'b' (строго > 0 по ОДЗ)
        self.slider_b = Slider(plt.axes([0.15, 0.04, 0.45, 0.025], facecolor=ax_color), 'Число b', 0.1, 10.0, valinit=self.b_init, valstep=0.1)
    ...
    def update(self, val):
        a = round(self.slider_a.val, 2)
        b = round(self.slider_b.val, 2)
        
        # --- ПРОВЕРКА ОДЗ ЛОГАРИФМА ---
        # Логарифм не существует, если основание a == 1
        if abs(a - 1.0) < 0.05:
            self.log_curve.set_visible(False)
            self.current_point.set_visible(False)
            self.proj_h.set_ydata([-100, -100])
            self.proj_v.set_xdata([-100, -100])
            self.info_text.set_text(
                f"НАРУШЕНИЕ ОДЗ:\n"
                f"----------------------------------------\n"
                f"Основание логарифма 'a' НЕ МОЖЕТ\n"
                f"быть равно 1 (a ≠ 1)!\n\n"
                f"Почему? Потому что 1 в любой степени\n"
                f"всё равно останется единицей.\n"
                f"Мы никогда не сможем получить число {b}."
            )
            self.fig.canvas.draw_idle()
            return
            
        # --- СТАНДАРТНЫЙ РАСЧЕТ ---
        self.log_curve.set_visible(True)
        self.current_point.set_visible(True)
        
        # Строим график функции y = log_a(x)
        # Используем формулу перехода к новому основанию: log_a(x) = ln(x) / ln(a)
        x_grid = np.linspace(0.01, 11, 500)
        y_grid = np.log(x_grid) / np.log(a)
        self.log_curve.set_data(x_grid, y_grid)
        
        # Вычисляем точное значение логарифма для выбранного числа b
        res_x = np.log(b) / np.log(a)
        
        # Двигаем красную точку на графике и её проекции
        self.current_point.set_data([b], [res_x])
        self.proj_h.set_ydata([res_x, res_x]); self.proj_h.set_xdata([-1, b])
        self.proj_v.set_xdata([b, b]); self.proj_v.set_ydata([-4, res_x])
        
        # Формируем наглядный разбор математической сути логарифма
        text_content = (
            f"ЗАПИСЬ ЛОГАРИФМА:\n"
            f" log_{a:.1f} ({b:.1f}) = ?\n\n"
            f"ПЕРЕВЕСТУ НА ЧЕЛОВЕЧЕСКИЙ ЯЗЫК:\n"
            f"Это просто вопрос-загадка:\n"
            f"«В какую СТЕПЕНЬ нужно возвести\n"
            f"число {a:.1f}, чтобы получить {b:.1f}?»\n\n"
            f"ПЕРЕВОД В ПРИВЫЧНУЮ СТЕПЕНЬ:\n"
            f" {a:.1f} в степени (?) = {b:.1f}\n"
            f" {a:.1f} ^ {res_x:.2f} = {b:.1f}\n\n"
            f"ОТВЕТ И СУТЬ:\n"
            f" Результат: {res_x:.2f}\n"
            f" Значит, log_{a:.1f} ({b:.1f}) = {res_x:.2f}\n\n"
            f"----------------------------------------\n"
            f"ПЕДАГОГИЧЕСКИЙ РАЗБОР:\n"
            f"Посмотри на красный пунктир!\n"
            f"1. Мы берем по оси X число {b:.1f}.\n"
            f"2. Поднимаемся к синему графику.\n"
            f"3. Смотрим налево на ось Y — там\n"
            f"   находится наш ответ: степень {res_x:.2f}."
        )
        
        # Добавляем вау-эффект для красивых целых степеней
        if abs(res_x - round(res_x)) < 0.01:
            text_content += f"\n\n🔥 КРАСИВЫЙ СЛУЧАЙ:\n {a:.0f} в степени {round(res_x)} это ровно {b:.0f}!"
            
        self.info_text.set_text(text_content)
        self.fig.canvas.draw_idle()

if __name__ == '__main__':
    app = LogarithmSimulationApp()
    plt.ioff()       
    plt.show()
...
class AlloysSimulationApp:
    def __init__(self):
        ...
        self.ax.axis('off')
        self.ax.set_title("Лаборатория текстовых задач: Сплавы металлов", fontsize=14, fontweight='bold', pad=15)
        
        # --- Стартовые параметры ---
        self.m1_init, self.w1_init = 10.0, 20.0  # Сплав 1: 10 кг, 20% меди
        self.m2_init, self.w2_init = 15.0, 70.0  # Сплав 2: 15 кг, 70% меди
        
        # --- Рисуем контуры брусков (Серые рамки) ---
        # Брусок 1 (слева)
        self.ax.add_patch(Rectangle((0.5, 0), 2.5, 100, edgecolor='dimgray', facecolor='#e2e8f0', lw=2, zorder=1))
        self.ax.text(1.75, 103, "Сплав 1", fontsize=11, fontweight='bold', ha='center', color='#b45309')
        
        # Брусок 2 (справа)
        self.ax.add_patch(Rectangle((10.0, 0), 2.5, 100, edgecolor='dimgray', facecolor='#e2e8f0', lw=2, zorder=1))
        self.ax.text(11.25, 103, "Сплав 2", fontsize=11, fontweight='bold', ha='center', color='#1d4ed8')
        
        # Брусок 3 (Итоговый сплав посередине, он шире)
        self.ax.add_patch(Rectangle((4.5, 0), 4.5, 100, edgecolor='black', facecolor='#cbd5e1', lw=2.5, zorder=1))
        self.ax.text(6.75, 103, "ИТОГОВЫЙ СПЛАВ", fontsize=12, fontweight='bold', ha='center', color='#15803d')
        
        # --- Заготовки для отображения чистой МЕДИ внутри брусков ---
        # Медь в 1-м бруске (медный/оранжевый цвет)
        self.copper1 = Rectangle((0.5, 0), 2.5, 0, edgecolor='none', facecolor='#ea580c', zorder=2)
        self.ax.add_patch(self.copper1)
        
        # Медь во 2-м бруске
        self.copper2 = Rectangle((10.0, 0), 2.5, 0, edgecolor='none', facecolor='#ea580c', zorder=2)
        self.ax.add_patch(self.copper2)
        
        # Медь в итоговом бруске
        self.copper3 = Rectangle((4.5, 0), 4.5, 0, edgecolor='none', facecolor='#ea580c', zorder=2)
        self.ax.add_patch(self.copper3)
        
        # Текстовые метки процентов и массы прямо на брусках
        self.txt_w1 = self.ax.text(1.75, 50, '', ha='center', va='center', fontsize=11, fontweight='bold', color='black', zorder=3)
        self.txt_w2 = self.ax.text(11.25, 50, '', ha='center', va='center', fontsize=11, fontweight='bold', color='black', zorder=3)
        self.txt_w3 = self.ax.text(6.75, 50, '', ha='center', va='center', fontsize=13, fontweight='bold', color='black', zorder=3)
        ...
        # Массы от 2 до 30 кг, проценты от 0 до 100%
        self.slider_m1 = Slider(self.ax_m1, 'Масса M1 (кг)', 2.0, 30.0, valinit=self.m1_init, valstep=1)
        self.slider_w1 = Slider(self.ax_w1, 'Медь W1 (%)', 0.0, 100.0, valinit=self.w1_init, valstep=5)
        self.slider_m2 = Slider(self.ax_m2, 'Масса M2 (кг)', 2.0, 30.0, valinit=self.m2_init, valstep=1)
        self.slider_w2 = Slider(self.ax_w2, 'Медь W2 (%)', 0.0, 100.0, valinit=self.w2_init, valstep=5)
    ...
    def update(self, val):
        ...
        # --- Математический расчет сплава ---
        # 1. Находим массу чистой меди в каждом исходном сплаве
        pure_cu1 = m1 * (w1 / 100.0)
        pure_cu2 = m2 * (w2 / 100.0)
        
        # 2. Итоговые параметры после переплавки
        m_total = m1 + m2
        pure_cu_total = pure_cu1 + pure_cu2
        w_total = (pure_cu_total / m_total) * 100.0
        
        # --- Визуальное отображение размеров брусков и меди ---
        # Максимальная высота бруска рассчитана на 30 кг. Переводим массу в высоту (0-100)
        h1 = (m1 / 30.0) * 100
        h2 = (m2 / 30.0) * 100
        # Итоговый брусок шире, его высота — это среднее заполнение относительно максимума в 60 кг
        h3 = (m_total / 60.0) * 100
        
        # Высота заполнения чистой медью внутри каждого бруска (масса меди / 30 * 100)
        # Она занимает нижнюю часть бруска, наглядно показывая долю
        self.copper1.set_height(h1 * (w1 / 100.0))
        self.copper2.set_height(h2 * (w2 / 100.0))
        self.copper3.set_height(h3 * (w_total / 100.0))
        
        # Сдвигаем бруски (серые подложки) по высоте в зависимости от массы
        # Чтобы зафиксировать их, мы просто меняем высоту Rectangle
        # Сами рамки стаканов/брусков статичны (100 пунктов), но мы закрашиваем внутреннюю массу сплава
        
        # Изменяем прозрачность или оттенок меди в итоговом сплаве в зависимости от концентрации
        alpha_total = np.clip(w_total / 100.0, 0.2, 1.0) if w_total > 0 else 0.1
        self.copper3.set_facecolor((0.92, 0.40, 0.16, alpha_total)) # RGBA цвет меди
        
        # Текстовые метки
        self.txt_w1.set_position((1.75, -8))
        self.txt_w1.set_text(f"{m1:.0f} кг\n({w1:.0f}%)")
        
        self.txt_w2.set_position((11.25, -8))
        self.txt_w2.set_text(f"{m2:.0f} кг\n({w2:.0f}%)")
        
        self.txt_w3.set_position((6.75, -8))
        self.txt_w3.set_text(f"{m_total:.0f} кг\n({w_total:.1f}%)")
        ...
        text_content = (
            ...
            f"----------------------------------------\n"
            f"Сплав 1:\n"
            f"Общая масса (M1) = {m1:.0f} кг\n"
            f"Процент меди (W1) = {w1:.0f}%\n"
            f"Чистая медь = {m1:.0f} * {w1/100:.2f} = {pure_cu1:.2f} кг\n\n"
            
            f"Сплав 2:\n"
            f"Общая massа (M2) = {m2:.0f} кг\n"
            f"Процент меди (W2) = {w2:.0f}%\n"
            f"Чистая медь = {m2:.0f} * {w2/100:.2f} = {pure_cu2:.2f} кг\n\n"
            
            f"ЛОГИКА СОСТАВЛЕНИЯ УРАВНЕНИЯ:\n"
            f"----------------------------------------\n"
            f"1. Итоговая масса нового сплава:\n"
            f"M_общ = M1 + M2\n"
            f"M_общ = {m1:.0f} + {m2:.0f} = {m_total:.0f} кг\n\n"
            
            f"2. Сколько ВСЕГО меди попало в печь:\n"
            f"Медь_общ = Медь1 + Медь2\n"
            f"Медь_общ = {pure_cu1:.2f} + {pure_cu2:.2f} = {pure_cu_total:.2f} кг\n\n"
            
            f"3. Вычисление итогового процента:\n"
            f"W_итог = (Медь_общ / M_общ) * 100%\n"
            f"W_итог = ({pure_cu_total:.2f} / {m_total:.0f}) * 100%\n\n"
            
            f"ОТВЕТ ЗАДАЧИ:\n"
            f"Концентрация меди в сплаве = {w_total:.2f}%"
        )
        self.info_text.set_text(text_content)
        self.fig.canvas.draw_idle()

if __name__ == '__main__':
    app = AlloysSimulationApp()
    plt.ioff()       
    plt.show()
